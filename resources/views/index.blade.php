...
<div>
    ...
    <main>
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
                <div class="font-semibold text-xl text-gray-800 leading-tight">@lang('xero-oauth-translations.label.header')</div>

            @if (!$token || $token->toOAuth2Token()->hasExpired())
                <div>
                    <p><i>@lang('xero-oauth-translations.status.unauthorized')</i></p>
                    <a href="{{ route('xero.auth') }}"
                       class="text-blue-400 underline">@lang('xero-oauth-translations.button.authorize')</a>
                </div>
            @else
                <p><i>@lang('xero-oauth-translations.status.authorized') </i></p>
            @endif
            <hr class="mb-2"/>
            <h3 class="mt-2">@lang('xero-oauth-translations.label.accounts')</h3>
            <div class="mt-2 flex">
